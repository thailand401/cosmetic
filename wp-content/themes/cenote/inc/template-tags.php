...
if ( ! function_exists( 'cenote_posted_on' ) ) :
	...
	function cenote_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( '%s', 'post date', 'cenote' ), // phpcs:ignore
...
if ( ! function_exists( 'cenote_posted_by' ) ) :
	...
	function cenote_posted_by() {
		$byline = sprintf(
			...
			esc_html_x( '%s', 'post author', 'cenote' ), // phpcs:ignore 
...
if ( ! function_exists( 'cenote_entry_footer' ) ) :
	...
	function cenote_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( '', 'list item separator', 'cenote' ) ); // phpcs:ignore
			if ( $tags_list ) {
...
function cenote_post_categories() {
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list ) {
			/* translators: 1: list of categories. */
			printf( '<span class="cat-links">' . esc_html__( '%1$s', 'cenote' ) . '</span>', $categories_list ); // phpcs:ignore
		}
	}
}
